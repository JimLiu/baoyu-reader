...
English and Chinese are kept together in this file so the release history stays
in sync.

本文件同时维护英文和中文内容，避免两个 changelog 逐步漂移。
...
- Renamed the package and CLI from `baoyu-markdown` to `baoyu-reader`.
...
- 将包名和 CLI 名称从 `baoyu-markdown` 更名为 `baoyu-reader`。
